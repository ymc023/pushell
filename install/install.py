...
class PreSetup(object):
    def __init__(self):
        ...
        self.db_port = 3306
        self.db_user = 'pushell'
        self.db_pass = 'pushell'
        self.db = 'pushell'
        self.mail_host = 'smtp.163.com'
        self.mail_port = 25
        self.mail_addr = 'cpis_serverM@163.com'
        self.mail_pass = 'ch123456'
        self.ip = ''
    ...
    def check_platform(self):
        if not (self._is_redhat or self._is_ubuntu):
            print(u"Allow platform: CentOS, RedHat, Fedora, Debian, Ubuntu, Amazon Linux.")
            exit()
    ...
    def write_conf(self, conf_file=os.path.join(jms_dir, 'pushell.conf')):
        color_print('写配置文件', 'green')
    ...
    def _set_env(self):
        color_print('开始检查防火墙和selinux', 'green')
        if self._is_redhat:
            os.system("export LANG='en_US.UTF-8'")
            if self._is_centos7 or self._is_fedora_new:
                cmd1 = "systemctl status firewalld 2> /dev/null 1> /dev/null"
                #cmd2 = "firewall-cmd --permanent --add-rich-rule='rule \
                #family="ipv4" port port="8000" protocol="tcp" accept'"
    ...
    def _require_pip(self):
        color_print('设置使用douabn的pip源', 'green')
        bash('mkdir /root/.pip')
        bash('cp pip.conf /root/.pip/pip.conf')
        #conf = ConfigParser.ConfigParser()
        #conf.read("/root/.pip/pip.conf")
        #conf.set('global','index-url','https://pypi.douban.com/simple')
        #conf.set('global','trusted-host','pypi.douban.com')
        ...
        self.check_bash_return(ret_code, "安装pushell依赖的python库失败！")
    ...
    def _input_mysql(self):
        while True:
            ...
            if mysql != 'n':
                ...
                db_user = raw_input('请输入数据库服务器用户 [pushell]: ').strip()
                db_pass = raw_input('请输入数据库服务器密码: ').strip()
                db = raw_input('请输入使用的数据库 [pushell]: ').strip()
