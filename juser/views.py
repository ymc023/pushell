...
def user_add(request):
    ...
    user_role = {'SU': u'超级管理员', 'CU': u'普通用户','AU':u'审计管理员','PU':u'配置管理员'}
...
def user_list(request):
    user_role = {'SU': u'超级管理员', 'GA': u'组管理员', 'CU': u'普通用户','AU':u'审计管理员','PU':'配置管理员'}
...
def forget_password(request):
    if request.method == 'POST':
        ...
        if user:
            ...
            send_mail('忘记pushell密码', msg, MAIL_FROM, [email], fail_silently=False)
...
def user_edit(request):
    ...
    if request.method == 'GET':
        ...
        user_role = {'SU': u'超级管理员', 'CU': u'普通用户','AU':'配置管理员','PU':'配置管理员'}
        user = get_object(User, id=user_id)
        group_all = UserGroup.objects.all()
        if user:
        ...
        user_role = {'SU': u'超级管理员', 'GA': u'部门管理员', 'CU': u'普通用户','AU':'审计管理员','PU':'配置管理员'}
...
def regen_ssh_key(request):
    ...
    return HttpResponse('ssh-key已生成，密码为 %s, 请到下载页面下载' % ssh_key_pass)
    ...
    if uuid_r:
        ...
        if user:
            ...
            if os.path.isfile(private_key_file):
                ...
                return response
    return HttpResponse('没有key文件，请联系管理员处理.')
