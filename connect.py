...
os.environ['DJANGO_SETTINGS_MODULE'] = 'pushell.settings'
if not django.get_version().startswith('1.6'):
    setup = django.setup()
from django.contrib.sessions.models import Session
from pushell.api import ServerError, User, Asset, PermRole, AssetGroup, get_object, mkdir, get_asset_info
from pushell.api import logger, Log, TtyLog, get_role_key, CRYPTOR, bash, get_tmp_dir
from jperm.perm_api import gen_resource, get_group_asset_perm, get_group_user_perm, user_have_perm, PermRole
from pushell.settings import LOG_DIR, NAV_SORT_BY
...
class Tty(object):
    ...
    def get_connection(self):
        ...
        try:
            """ 
            这段注释是使用ssh-key进行连接主机，目前只启用密码连接主机。2016.09.22注释，暂不启用 
...
*                        Welcome to Pushell !                                 *
*                                                                             *
*    Please Enter your command ! Press exit  to abort !                       *
*******************************************************************************\033[0m
 ...
 quit      --- (Quit)
    ...
    def print_search_result(self):
        ...
        color_print(line % ('ID', 'IP', 'PORT', 'HOSTNAME', 'SYSTEMNAME','COMMENT'), 'title')
    ...
    def exec_cmd(self):
        ...
        while True:
            ...
            if len(roles) > 1:  # 授权角色数大于1
                ...
                print
                print "请输入运行命令所关联系统用户的ID, quit退出"
            ...
            for asset in assets:
                ...
                print 'assets arg 包含有其他方法:%s ' %s(dir(assets))
            print
            print "请输入Hostname或ansible支持的参数, 多个主机:分隔, quit退出"
            pattern = raw_input("\033[1;32m<Pushell_PEXE>:\033[0m ").strip()
            if (pattern =='quit'):
                break
            else:
                print "your input args pattern is:%s" %s(pattern)
                res = gen_resource({'user': self.user, 'asset': assets, 'role': role}, perm=self.user_perm)
                print "your res is:%s"%(res)
                runner = MyRunner(res)
                asset_name_str = ''
                print "匹配主机:"
    ...
    def upload(self):
        while True:
            try:
                print "进入批量上传模式"
                print "请输入主机名或ansible支持的pattern, 多个主机:分隔 quit退出"
                pattern = raw_input("\033[1;32m<Pushell_EXEC>:\033[0m ").strip()
                if pattern == 'quit':
                    break
                else:
...
def main():
    ...
    if not login_user:  # 判断用户是否存在
        color_print('用户名错误，请联系管理员处理.', exits=True)

    if not login_user.is_active:
        color_print('你的用户已禁用，请联系管理员处理.', exits=True)
    if login_user.role != 'CU':
        color_print('你无权进行此操作，请联系管理员处理.',exits=True)
    ...
    try:
        while True:
            try:
                option = raw_input("\033[1;32m[pushell]:\033[0m ").strip()
            except EOFError:
                nav.print_nav()
                continue
            except KeyboardInterrupt:
                sys.exit(0)
            if option.startswith(('show','SHOW','Show','SHOw','sHOW')):
                nav.search()
                nav.print_search_result()
                continue
            elif option.startswith(('?','h','hel','help','Help','HELP','ls','dir','H')):
                nav.print_nav()
                ...
                continue
            elif option.startswith('pexe'):
                nav.exec_cmd()
                continue
            elif option.startswith('pput'):
                nav.upload()
            elif option.startswith('pget'):
                nav.download()
            elif option in [ 'quit', 'exit']:
                sys.exit()
            elif option.startswith(('ssh','SSH')) and len(option) == 5:
                nav.search(option[4])
                if len(nav.search_result) == 1:
                    print('Only match Host:  %s ' % nav.search_result[0].hostname)
                    nav.try_connect()
            else:
                print ("Unrecognized command.please try ? or help !")
                #nav.search(option)
                #if len(nav.search_result) == 1:
                    #print('Only match Host:  %s ' % nav.search_result[0].hostname)
                    #nav.try_connect()
