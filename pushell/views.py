...
from two_auth import TwoAuth
from pushell.models import Setting
...
def is_latest():
    ...
    lastest_version = urllib.urlopen('http://www.pushell.org/lastest_version.html?jsn=%s' % jsn).read().strip()
...
def Login(request):
    ...
    if request.method == 'GET':
        ...
        try:
            if request.POST.get('token') is None or request.POST.get('username') is None or request.POST.get('password') is None:
                error = '缺少必填参数'
            try:
                int(request.POST.get('token'))
            except Exception:
                error= 'Token 类型错误'
            else:
                username = request.POST.get('username')
                password = request.POST.get('password')
                getcode = request.POST.get('token')
                two=TwoAuth()
                if two.verify(getcode):
